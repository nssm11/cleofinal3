    ...
    <div className="mx-auto w-full max-w-[96rem] px-3 sm:px-5 lg:px-7">
      <header className="flex flex-wrap items-end justify-between gap-4 pb-4 pt-5">
        <div className="min-w-0">
          <p className="os-label text-ops-faint">Système · Diagnostic</p>
          <h1 className="mt-1.5 font-ant uppercase text-[clamp(1.6rem,3.6vw,2.4rem)] leading-[1.02] tracking-tight text-ops-ink">Santé du système</h1>
          <p className="mt-1 max-w-[64ch] text-[13px] text-ops-muted">MesurÃ© Ã  l’instant de l’ouverture de cet Ã©cran â rien n’est en cache, rien n’est inventÃ©.</p>
        </div>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Sheet>
          <p className="os-label text-ops-muted">Réponse de la base</p>
          <p className="os-num mt-1.5 font-ant uppercase text-[2rem] leading-none text-ops-ink">{latency.ok ? `${latency.ms} ms` : "—"}</p>
          <p className="mt-1.5 text-[11.5px] leading-relaxed text-ops-muted">
            ...
            {!latency.ok && latency.error && <span className="mt-1 block font-mono text-[10.5px] text-crit">{latency.error}</span>}
          </p>
        </Sheet>

        <Sheet>
          <p className="os-label text-ops-muted">File de lettres · 30 jours</p>
          <p className="os-num mt-1.5 font-ant uppercase text-[2rem] leading-none text-ops-ink">{mail.pending}</p>
          <p className="mt-1.5 text-[11.5px] leading-relaxed text-ops-muted">
            en attente d’envoi Â· <span className={mail.failed ? "text-crit" : ""}>{mail.failed} en Ã©chec</span> Â· {mail.sent} livrÃ©e(s)
          </p>
          <Link href="/admin/emails" className="mt-2.5 inline-flex items-center gap-1 text-[11px] uppercase tracking-[0.12em] text-ops-signal">Ouvrir les opérations e-mail <Glyph name="arrowRight" size={11} /></Link>
        </Sheet>

        <Sheet>
          <p className="os-label text-ops-muted">Paiements refusés</p>
          <p className="os-num mt-1.5 font-ant uppercase text-[2rem] leading-none text-ops-ink">{failedPayments.length ? "en cours" : "0"}</p>
          <p className="mt-1.5 text-[11.5px] leading-relaxed text-ops-muted">
          ...
          <Link href="/admin/attention" className="mt-2.5 inline-flex items-center gap-1 text-[11px] uppercase tracking-[0.12em] text-ops-signal">Centre d’attention <Glyph name="arrowRight" size={11} /></Link>
        </Sheet>

        <Sheet>
          <p className="os-label text-ops-muted">Score qualité du catalogue</p>
          <p className="os-num mt-1.5 font-ant uppercase text-[2rem] leading-none text-ops-ink">{quality.score}</p>
          <p className="mt-1.5 text-[11.5px] leading-relaxed text-ops-muted">{quality.issues.length} point(s) relevé(s) sur {quality.scanned} fiches · {quality.checks} contrôles</p>
          <Link href="/admin/produits/qualite" className="mt-2.5 inline-flex items-center gap-1 text-[11px] uppercase tracking-[0.12em] text-ops-signal">Ouvrir l’audit <Glyph name="arrowRight" size={11} /></Link>
        </Sheet>
      </section>
        ...
        <Sheet padded={false}>
          ...
          <dl className="grid grid-cols-2 gap-x-6 gap-y-1.5 px-4 py-3 text-[12.5px] sm:grid-cols-3">
            {registres.map(([label, value]) => (
              <div key={label} className="flex items-baseline justify-between gap-2 border-b border-dashed border-ops-line-soft py-1.5">
                <dt className="text-ops-muted">{label}</dt>
                <dd className="os-num text-ops-ink">{new Intl.NumberFormat("fr-TN").format(value)}</dd>
              </div>
            ))}
          </dl>
        </Sheet>
        ...
        <div className="grid content-start gap-3">
          <Sheet>
            ...
            <ul className="mt-3 space-y-2.5 text-[12.5px]">
              <li className="flex items-start justify-between gap-3 border-b border-dashed border-ops-line-soft pb-2.5">
                <span className="text-ops-ink">Base de données</span>
                <Tag tone={latency.ok ? "good" : "bad"}>{latency.ok ? "joignable" : "en difficulté"}</Tag>
              </li>
              <li className="flex items-start justify-between gap-3 border-b border-dashed border-ops-line-soft pb-2.5">
                <span className="text-ops-ink">Courrier (file d’envoi)</span>
                <Tag tone={mail.failed > 3 ? "bad" : mail.pending > 10 ? "warn" : "neutral"}>{mail.failed > 3 ? "sous pression" : mail.pending > 10 ? "chargée" : "calme"}</Tag>
              </li>
              <li className="flex items-start justify-between gap-3 border-b border-dashed border-ops-line-soft pb-2.5">
                <span className="text-ops-ink">Encaissement</span>
                <Tag tone="neutral">livraison · virement · carte · carte cadeau</Tag>
              </li>
              <li className="flex items-start justify-between gap-3">
                <span className="text-ops-ink">Suivi des colis</span>
                <Tag tone="neutral">17TRACK</Tag>
              </li>
            </ul>
            <p className="mt-3 border-t border-ops-line pt-3 text-[11.5px] leading-relaxed text-ops-faint">
      ...
      <section className="mt-3">
        <Sheet padded={false}>
          <div className="flex items-center justify-between gap-3 border-b border-ops-line px-4 py-3">
            ...
            <Link href="/admin/audit" className="shrink-0 text-[11px] uppercase tracking-[0.12em] text-ops-signal">Tout le journal</Link>
          </div>
            ...
            <ul className="divide-y divide-ops-line-soft">
              {recentAudit.map(({ a, actor }) => (
                <li key={a.id} className="flex flex-wrap items-center gap-x-4 gap-y-1 px-4 py-2.5 text-[12.5px]">
                  <span className="os-num w-32 shrink-0 text-ops-faint">{formatDateTime(a.createdAt)}</span>
                  <span className="min-w-0 flex-1 truncate text-ops-ink"><span className="font-mono text-[11.5px]">{a.action}</span> · {a.entity} #{a.entityId}</span>
                  <span className="shrink-0 text-ops-muted">{actor ?? "système"}</span>
                </li>
              ))}
            </ul>
          )}
