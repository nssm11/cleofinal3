  ...
  return (
    <AdminPage title="Clientes" sub={`${rows.length} affichées · ${s?.total ?? 0} comptes dans le registre`} action={
      ...
      <div className="mb-4 grid grid-cols-3 gap-3">
        <div className="border border-os-line bg-os-surface px-4 py-3 shadow-os-sheet"><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-os-muted">Comptes</p><p className="mt-1.5 font-display text-[1.5rem] leading-none text-os-text">{new Intl.NumberFormat("fr-TN").format(s?.total ?? 0)}</p></div>
        <div className="border border-os-line bg-os-surface px-4 py-3 shadow-os-sheet"><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-os-muted">Ont commandé</p><p className="mt-1.5 font-display text-[1.5rem] leading-none text-os-text">{new Intl.NumberFormat("fr-TN").format(s?.buyers ?? 0)}</p></div>
        <div className="border border-os-line bg-os-surface px-4 py-3 shadow-os-sheet"><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-os-muted">Nouvelles · 30 jours</p><p className="mt-1.5 font-display text-[1.5rem] leading-none text-os-text">{new Intl.NumberFormat("fr-TN").format(s?.fresh ?? 0)}</p></div>
      </div>
