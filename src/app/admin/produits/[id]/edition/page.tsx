...
export default async function AdminProductEdition({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (<AdminPage title={isNew ? "Nouveau produit" : `Édition — ${product!.name}`} sub={product ? `SKU ${product.sku} · les modifications sont journalisées` : undefined}><ProductForm product={product ?? undefined} brands={b} categories={c} concerns={k} selectedConcerns={sel} /></AdminPage>);
}
