...
export default async function AdminStock() {
  ...
  /* ── LA RUPTURE PRÉVUE ────────────────────────────────────────────────────
     « Il reste 6 » ne dit rien. « Il reste 6, et vous en vendez 2 par jour »
     dit tout : dans trois jours le rayon est vide. Le calcul est une division,
     la vitesse vient des ventes réelles des trente derniers jours, et rien
     n'est prédit pour une référence qui ne se vend pas. */
  // eslint-disable-next-line react-hooks/purity -- a server component reading the clock once, on purpose: the whole point of the row below is "when will this run out, from today".
  const nowMs = Date.now();
  const stockById = new Map(all.map((p) => [p.id, p.stock]));
  const forecast = velocity
    .filter((v) => v.perDay > 0.05)
    .map((v) => {
      const stock = stockById.get(v.productId) ?? 0;
      return { ...v, stock, days: stock / v.perDay };
    })
    .filter((v) => v.days <= 21)
  ...
  return (
    <AdminPage title="Stock" sub={`${low.length} alerte(s)`}>
      ...
      {forecast.length > 0 && (
        <div className="mb-8">
          <h2 className="mb-3 text-[10px] uppercase tracking-[0.16em] text-ops-muted">Rupture prévue — d&apos;après vos ventes réelles des 30 derniers jours</h2>
          <Table head={["Produit", "Stock", "Ventes / jour", "Jours restants", "Épuisé vers le", "À commander"]} minWidth="min-w-[860px]">
            {forecast.map((f) => {
              const when = new Date(nowMs + f.days * 86_400_000);
              return (
                <tr key={f.productId} className="border-b border-ops-line/60 last:border-0">
                  <td className="px-4 py-2.5"><Link href={`/admin/produits/${f.productId}`} className="hover:underline">{all.find((p) => p.id === f.productId)?.name ?? `#${f.productId}`}</Link></td>
                  <td className="px-4 py-2.5 tabular-nums">{f.stock}</td>
                  <td className="px-4 py-2.5 tabular-nums text-ops-muted">{f.perDay.toFixed(2)}</td>
                  <td className={`px-4 py-2.5 tabular-nums ${f.days <= 5 ? "text-crit" : f.days <= 12 ? "text-amber" : "text-ops-muted"}`}>{f.days < 1 ? "moins d'un jour" : `${Math.floor(f.days)} j`}</td>
                  <td className="px-4 py-2.5 text-ops-muted">{when.toLocaleDateString("fr-TN", { day: "2-digit", month: "short" })}</td>
                  <td className="px-4 py-2.5 tabular-nums">{Math.max(6, Math.ceil(f.perDay * 30))} unités (1 mois)</td>
                </tr>
              );
            })}
          </Table>
          <p className="mt-2 text-[11px] text-ops-muted">
            La quantité conseillée couvre trente jours au rythme observé. Aucune prédiction n&apos;est faite pour une référence qui ne se vend pas — {formatDT(0)} de ventes ne se projette pas.
          </p>
        </div>
      )}
