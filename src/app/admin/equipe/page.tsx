    ...
    <div className="mx-auto w-full max-w-[96rem] px-3 sm:px-5 lg:px-7">
      <header className="flex flex-wrap items-end justify-between gap-4 pb-4 pt-5">
        <div className="min-w-0">
          <p className="os-label text-ops-faint">Système · Accès</p>
          <h1 className="mt-1.5 font-ant uppercase text-[clamp(1.6rem,3.6vw,2.4rem)] leading-[1.02] tracking-tight text-ops-ink">Équipe &amp; accès</h1>
              ...
              return (
                <li key={u.id} className="flex flex-wrap items-center gap-x-4 gap-y-3 px-4 py-3.5">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="flex flex-wrap items-center gap-2 text-[14px] text-ops-ink">
                      {u.firstName} {u.lastName}
                      <Tag tone={isAdmin ? "gold" : "info"}>{isAdmin ? "Administratrice" : "Support"}</Tag>
                      {u.id === me.id && <Tag tone="neutral">vous</Tag>}
                    </p>
                    <p className="mt-0.5 truncate text-[12px] text-ops-muted">{u.email}{u.phone ? ` · ${u.phone}` : ""}</p>
                  </div>
                  <div className="flex shrink-0 items-center gap-5 text-[12px]">
                    <span className="text-right">
                      <span className="os-num block font-ant uppercase text-[1.2rem] leading-none text-ops-ink">{openTasks(u.id)}</span>
                      <span className="text-[10px] uppercase tracking-[0.12em] text-ops-faint">tâches ouvertes</span>
                    </span>
                    <span className="text-right">
                      <span className="os-num block font-ant uppercase text-[1.2rem] leading-none text-ops-ink">{liveSessions(u.id)}</span>
                      <span className="text-[10px] uppercase tracking-[0.12em] text-ops-faint">session(s)</span>
                    </span>
                    <span className="hidden w-44 text-right sm:block">
                      <span className="os-num block text-[12px] leading-snug text-ops-ink">{lastAction(u.id) ? formatDateTime(lastAction(u.id)!.createdAt) : "—"}</span>
                      <span className="text-[10px] uppercase tracking-[0.12em] text-ops-faint">dernier geste</span>
                    </span>
                  </div>
                </li>
              );
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-3 border border-ops-line bg-ops-sheet-2/50 px-4 py-3 text-[11px] text-ops-muted">
        <span className="flex items-center gap-2">
          <Glyph name="lock" size={13} className="text-ops-signal" />
        ...
        <span className="flex items-center gap-3">
          <Link href="/admin/taches" className="uppercase tracking-[0.12em] text-ops-signal">Tâches</Link>
          <Link href="/admin/audit" className="uppercase tracking-[0.12em] text-ops-signal">Journal d’audit</Link>
        </span>
      </div>
    </div>
  );
