...
export default async function MarquesPage() {
  ...
  return (
    <div>
      <section className="relative overflow-hidden bg-canvas pb-14 pt-28 lg:pb-20 lg:pt-36">
        <MotifLayer motif="architecture" mark={[78, 12]} />
      ...
      {/* The wall */}
      <section className="relative overflow-hidden border-y border-line/70 bg-mist">
          ...
          <ul className="flex flex-wrap items-baseline gap-x-10 gap-y-5 lg:gap-x-16 lg:gap-y-6">
            {featured.map((b, i) => (
              <Reveal key={b.id} as="li" y={12} delay={i * 0.04}>
                <Link
                  href={`/marque/${b.slug}`}
                  className={`group inline-flex items-baseline gap-3 transition-colors duration-500 ${ i % 3 === 0 ? "font-ant uppercase text-[clamp(1.9rem,3.8vw,3.1rem)] text-carbon" : i % 3 === 1 ? "font-ant uppercase text-[clamp(1.4rem,2.4vw,2rem)] text-carbon" : "font-ant uppercase text-[clamp(1.1rem,1.8vw,1.5rem)] text-carbon" }`}
                >
                  <span className="relative">
                    {b.name}
                    <span
                      aria-hidden
                      className="absolute -bottom-0.5 left-0 h-px w-full origin-right scale-x-0 bg-iodine-deep transition-transform duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:origin-left group-hover:scale-x-100"
                    />
                  </span>
                  <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-faint">{b.country}</span>
                </Link>
              </Reveal>
            ))}
          </ul>
      ...
      <section className="shell-wide py-block lg:py-block-lg">
        <p className="kicker-xs mb-10 text-faint">Le répertoire complet</p>
        <div className="grid gap-x-14 gap-y-12 sm:grid-cols-2 lg:grid-cols-3">
          {Object.entries(groups)
            .sort()
            .map(([letter, bs]) => (
              <div key={letter}>
                <p className="flex items-baseline gap-4 border-b border-line/70 pb-3">
                  <span className="font-ant uppercase text-[30px] leading-none text-iodine-deep">{letter}</span>
                ...
                <ul className="mt-1">
                  {bs.map((b) => (
                    <li key={b.id} className="border-b border-line/50">
