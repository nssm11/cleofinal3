...
export default async function MarquesPage() {
  ...
  return (
    <div>
      <section className="relative overflow-hidden bg-porcelain pb-14 pt-28 lg:pb-20 lg:pt-36">
        ...
        <div className="relative container-wide">
          ...
          <Reveal y={14} amount={0.1}>
            <h1 className="max-w-[22ch] font-display text-[clamp(2.4rem,5.6vw,4.6rem)] leading-[0.95] tracking-[-0.028em] text-ink">
              Seize maisons qui
              <span className="italic text-cinabre-2"> engagent leur nom.</span>
            </h1>
            <p className="mt-8 max-w-[40rem] text-[15.5px] leading-[1.85] text-graphite">
      ...
      {/* The wall */}
      <section className="relative overflow-hidden border-y border-rule/70 bg-bone">
        <div aria-hidden className="pointer-events-none absolute inset-0">
          <div className="marble-veil opacity-35" />
        </div>
        <div className="relative container-wide py-rhythm lg:py-rhythm-lg">
          <p className="eyebrow mb-10 text-ash">Les maisons invitées</p>
                ...
                <Link
                  ...
                  className={`group inline-flex items-baseline gap-3 transition-colors duration-500 ${
                    i % 3 === 0
                      ...
                      : i % 3 === 1
                        ? "font-display text-[clamp(1.4rem,2.4vw,2rem)] text-slate"
                        : "font-display text-[clamp(1.1rem,1.8vw,1.5rem)] text-slate"
                  }`}
                >
                  <span className="relative">
                    ...
                    <span
                      aria-hidden
                      className="absolute -bottom-0.5 left-0 h-px w-full origin-right scale-x-0 bg-cinabre-2 transition-transform duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:origin-left group-hover:scale-x-100"
                    />
                  </span>
                  <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-ash">{b.country}</span>
                </Link>
              </Reveal>
            ))}
          </ul>

          <div className="mt-16 grid gap-10 border-t border-rule/70 pt-12 lg:grid-cols-3">
            {featured.slice(0, 3).map((b, i) => (
              <Reveal key={b.id} y={14} delay={i * 0.07}>
                <Link href={`/marque/${b.slug}`} className="group block">
                  <p className="font-display text-[21px] text-ink transition-colors duration-500 group-hover:text-cinabre-2">
                    {b.name}
                  </p>
                  <p className="mt-3 line-clamp-4 text-[13.5px] leading-relaxed text-graphite">{b.story}</p>
                  <span className="mt-4 inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.2em] text-cinabre-2">
      ...
      <section className="container-wide py-rhythm lg:py-rhythm-lg">
        <p className="eyebrow mb-10 text-ash">Le répertoire complet</p>
        <div className="grid gap-x-14 gap-y-12 sm:grid-cols-2 lg:grid-cols-3">
          {Object.entries(groups)
            .sort()
            .map(([letter, bs]) => (
              <div key={letter}>
                <p className="flex items-baseline gap-4 border-b border-rule/70 pb-3">
                  <span className="font-display text-[30px] italic leading-none text-cinabre-2">{letter}</span>
                ...
                <ul className="mt-1">
                  {bs.map((b) => (
                    <li key={b.id} className="border-b border-rule/50">
                      <Link href={`/marque/${b.slug}`} className="group flex items-baseline justify-between gap-3 py-3">
                        <span className="text-[14.5px] text-slate transition-colors group-hover:text-ink">
                          {b.name}
                        </span>
                        <span className="shrink-0 text-[10px] uppercase tracking-[0.16em] text-ash">{b.country}</span>
                      </Link>
                    </li>
                  ))}
                </ul>
