...
export default function CarteCadeauPage() {
  ...
  return (
    <div>
      ...
      <div className="shell-wide grid gap-12 py-16 lg:grid-cols-12 lg:py-24">
        <div className="lg:col-span-7">
          <ol className="space-y-0 border-t border-line/60">
            {notes.map((r, i) => (
              <Reveal key={r.n} y={14} delay={i * 0.06} amount={0.1}>
                <li className="grid gap-4 border-b border-line/60 py-8 sm:grid-cols-[3rem_1fr] sm:gap-8">
                  <span className="flex h-12 w-12 items-center justify-center rounded-full border border-iodine-deep/40 bg-iodine-wash/60 text-iodine-deep">
                    <r.i size={20} strokeWidth={1.3} />
                  </span>
                  <span>
                    <span className="flex items-baseline gap-4">
                      <span className="font-ant uppercase text-[13px] text-iodine-deep">{r.n}</span>
                      <span className="font-ant uppercase text-[clamp(1.3rem,2.6vw,1.7rem)] text-carbon">{r.t}</span>
                    </span>
          ...
          <Reveal y={12} className="mt-10 flex flex-wrap gap-3">
            <Link href="/boutiques" className="btn-outline">Nos boutiques</Link>
