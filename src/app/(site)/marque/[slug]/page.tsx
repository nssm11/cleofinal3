  ...
  return (
    <div>
      <section className="relative overflow-hidden bg-canvas pb-12 pt-28 lg:pb-16 lg:pt-36">
        <MotifLayer motif="architecture" mark={[16, 16]} />
        <div className="relative shell-wide">
          ...
          <div className="mt-12 grid gap-12 lg:grid-cols-12 lg:gap-16">
            <Reveal className="lg:col-span-7" y={14} amount={0.1}>
              <p className="kicker-xs mb-7">{b.country ?? "Laboratoire"}</p>
              <h1 className="font-ant uppercase text-[clamp(2.4rem,5.8vw,4.8rem)] leading-[0.94] tracking-[-0.03em] text-carbon">
                {b.name}
              </h1>
              {b.story && (
                <p className="mt-8 max-w-[42rem] text-[15.5px] leading-[1.9] text-muted">{b.story}</p>
              )}
              <div className="mt-9 flex flex-wrap items-center gap-x-8 gap-y-3 border-t border-line/70 pt-6 text-[12.5px] text-muted">
                <span className="flex items-center gap-2">
                  <ShieldIcon size={14} className="text-iodine-deep" /> Distribution officielle en Tunisie
                </span>
                {b.isFeatured && (
                  <span className="flex items-center gap-2 text-iodine-deep">
                    <span aria-hidden className="h-px w-5 bg-iodine-deep" /> Sélection maison
                  </span>
                )}
              </div>
            </Reveal>
            ...
            <Reveal className="lg:col-span-4 lg:col-start-9 lg:pt-6" y={14} delay={0.1}>
              <p className="kicker-xs mb-6 text-faint">Continuer</p>
              <ul className="border-t border-line/70">
                {[
                  ...
                  <li key={href} className="border-b border-line/70">
                    <Link
                      href={href}
                      className="group flex items-center justify-between gap-4 py-3.5 text-[14px] text-carbon transition-colors hover:text-carbon"
                    >
                      {label}
                      <ArrowRightIcon
                        size={13}
      ...
      {hero.length >= 2 && (
        <section className="shell-wide pb-14" aria-label={mm.brandHeroEyebrow}>
          <Reveal>
            <p className="kicker-xs mb-7 flex items-center gap-3 text-faint">
              <span aria-hidden className="h-px w-8 bg-iodine" />
              {mm.brandHeroEyebrow}
              <span className="text-iodine-deep">— {b.name}</span>
            </p>
