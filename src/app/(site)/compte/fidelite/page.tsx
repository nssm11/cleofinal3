  ...
  return (
    <div className="max-w-[64rem]">
          ...
          <div className={`${cardPad} grid gap-10 sm:grid-cols-[auto_1fr] sm:gap-14`}>
            <div>
              ...
              <p className="font-display text-[clamp(3rem,6vw,4.4rem)] leading-none text-ink">
                ...
                <span className="ms-3 text-[0.24em] font-bold uppercase tracking-[0.24em] text-ash">{t.points}</span>
              </p>
              <p className="mt-5 text-[12.5px] text-graphite">
                {t.lifetime.replace("{n}", summary.lifetime.toLocaleString("fr-FR"))}
              </p>
            </div>

            <div className="flex flex-col justify-center sm:border-l sm:border-rule/60 sm:pl-12">
              <div className="flex items-baseline justify-between gap-4">
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-ash">{t.nextReward}</p>
              ...
              <p className="mt-4 text-[13px] text-graphite">
                {untilNext === 0 ? (
                  <span className="font-medium text-cinabre-2">{t.rewardReady}</span>
                ) : (
          ...
          <AccountCard>
            <div className={cardPad}>
              <div className="flex flex-wrap items-end justify-between gap-4">
                <div>
                  <p className="rule-label mb-3 text-cinabre-2">{t.advantages}</p>
              ...
              <ul aria-label={t.advantages} className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {Array.from({ length: Math.min(summary.blocks, 6) }).map((_, i) => (
                  <li
                    key={i}
                    className="relative overflow-hidden border border-cinabre-2/35 bg-gradient-to-br from-cinabre-soft/70 via-alabaster to-bone px-5 py-4"
                  >
                    <span aria-hidden className="absolute inset-y-0 left-0 w-[2px] bg-cinabre-2" />
                    <p className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.2em] text-cinabre-2">
              ...
              {summary.blocks > 6 && (
                <p className="mt-4 text-[12.5px] text-graphite">+ {summary.blocks - 6} × {t.rewardValue}</p>
              )}
              <p className="mt-4 border-t border-rule/60 pt-4 text-[13px] leading-relaxed text-graphite">{t.advantagesHint}</p>
            </div>
          </AccountCard>
        </Reveal>
      )}
      ...
      <Reveal y={14} delay={0.08} amount={0.05} className="mt-6">
        <AccountCard>
          <div className={cardPad}>
            <p className="rule-label mb-5 text-cinabre-2">{t.howItWorks}</p>
            <p className="text-[14px] leading-[1.85] text-slate">{t.howText}</p>
            <p className="mt-5 flex items-start gap-3 border-t border-rule/60 pt-5 text-[13px] italic text-cinabre-2">
      ...
      <section className="mt-12">
        ...
        {summary.recent.length === 0 ? (
          <Reveal y={10} className="mt-7">
            <p className="rounded-[3px] border border-dashed border-rule-strong/70 bg-bone/50 px-6 py-12 text-center text-[13.5px] text-graphite">
              {t.noHistory}
            </p>
          </Reveal>
        ) : (
                ...
                {summary.recent.slice(0, 24).map((r) => (
                  <li key={r.id} className="flex items-baseline justify-between gap-5 px-6 py-4">
                    <span className="min-w-0">
                      <span className="block truncate text-[13.5px] text-slate">{r.reason}</span>
                      <span className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[11.5px] text-ash">
                        <span
                          className={`font-bold uppercase tracking-[0.14em] ${r.points > 0 ? "text-success" : "text-cinabre-2"}`}
                        >
                          {kindLabel(r.kind)}
                        </span>
                        {r.orderNumber && (
                    ...
                    <span className="flex shrink-0 items-baseline gap-5">
                      <span
                        className={`text-[14px] font-medium tabular-nums ${r.points > 0 ? "text-success" : "text-cinabre"}`}
                      >
                        {r.points > 0 ? "+" : ""}
                        {r.points.toLocaleString("fr-FR")}
                      </span>
                      <span className="hidden w-28 text-right text-[11.5px] tabular-nums text-ash sm:block">
