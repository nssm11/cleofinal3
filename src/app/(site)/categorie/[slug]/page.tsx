  ...
  return (
    <div>
      ...
      <section className="relative">
        {heroImage ? (
          <div className="relative h-[52vh] min-h-[420px] w-full overflow-hidden bg-cine-noir lg:h-[62vh]">
            ...
            <div className="absolute inset-x-0 bottom-0">
              <div className="container-wide pb-12 lg:pb-16">
                ...
                {c.description && <p className="mt-5 max-w-[42ch] text-[14px] leading-[1.8] text-cine-mist">{c.description}</p>}
              </div>
            </div>
          </div>
        ) : (
          <div className="bg-cine-noir">
      ...
      {/* ══ THE EDITORIAL ROW ═══════════════════════════════════════════ */}
      <section className="border-b border-stone/60 bg-paper">
        <div className="container-wide grid gap-10 py-14 lg:grid-cols-12 lg:gap-16 lg:py-20">
          <Reveal className="lg:col-span-5" y={12}>
            {c.parent?.story && (
              <>
                ...
                <p className="text-[15px] leading-[1.95] text-charcoal">{c.parent.story}</p>
              </>
            )}
          </Reveal>
          ...
          {siblings.length > 0 && (
            <Reveal className="lg:col-span-6 lg:col-start-7" y={12} delay={0.08}>
              <p className="eyebrow mb-6 text-muted-2">{fmt(t.alsoIn, { name: c.parent?.name ?? "" })}</p>
              <ul className="grid grid-cols-2 gap-x-8 gap-y-4">
                {siblings.map((s) =>
                  s.slug === c.slug ? (
                    <li key={s.id}>
                      <span aria-current="page" className="inline-flex items-baseline gap-3 font-display text-[18px] font-light text-champagne-2">
                    ...
                    <li key={s.id}>
                      <Link
                        ...
                        className="group inline-flex items-baseline gap-3 font-display text-[18px] font-light text-charcoal transition-colors hover:text-ink"
                      >
                        <span aria-hidden className="h-px w-5 bg-stone-2 transition-all duration-500 group-hover:w-7 group-hover:bg-champagne-2" />
                        {s.name}
                      </Link>
                    </li>
                  ),
      ...
      {/* ══ THE CURATED ROW ═════════════════════════════════════════════ */}
      {curatedRow.length > 0 && (
        <section className="border-b border-stone/60 bg-cream/50">
          <div className="container-wide py-14 lg:py-20">
            <Reveal>
              <div className="mb-10 flex flex-wrap items-end justify-between gap-5">
                <div>
                  <p className="eyebrow mb-4 text-muted-2">Sélection</p>
      ...
      <section className="border-t border-stone/60 bg-cream/50">
