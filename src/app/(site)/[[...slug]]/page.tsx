...
async function Home() {
  const [universes, featured, novelties, posts, brandRows, bestsellers] = await Promise.all([
    getUniverses(), getFeatured(8), getNewArrivals(4), db.select().from(articles).where(eq(articles.isPublished, true)).orderBy(desc(articles.publishedAt)).limit(3), db.select().from(brands).where(eq(brands.isFeatured, true)).limit(8), listProducts({ perPage: 8, sort: "bestsellers" }),
  ]);
  return <div className="rb-home">
    ...
    <RebuildTrustBar />
    <section className="rb-proof-strip"><div><span className="rb-proof-number">81</span><div><b>références actives</b><p>Une sélection tenue à jour, sans catalogue infini.</p></div></div><div><span className="rb-proof-number">02</span><div><b>comptoirs à Tunis</b><p>Ezzahra et Hammam-Lif, pour parler en vrai.</p></div></div><div><span className="rb-proof-number">24–72</span><div><b>heures de livraison</b><p>Partout en Tunisie, avec un suivi lisible.</p></div></div></section>
    ...
    <section className="rb-section rb-quick-shop"><div className="rb-section-heading"><div><span className="rb-kicker">03 / LE COMPTOIR</span><h2>Les gestes<br /><em>les plus choisis.</em></h2></div><div><p>Les références qui reviennent le plus souvent dans les paniers du comptoir, sélectionnées pour leur efficacité et leur tolérance.</p><Link href="/boutique?sort=bestsellers" className="rb-text-link">Voir les meilleures ventes <span>↗</span></Link></div></div><RebuildProductGrid products={bestsellers.items} /></section>
    <section className="rb-section rb-selected"><div className="rb-section-heading"><div><span className="rb-kicker">04 / LA SÉLECTION</span><h2>Une table<br /><em>bien composée.</em></h2></div><Link href="/boutique" className="rb-button rb-button-line">Tout voir <span>↗</span></Link></div><div className="rb-feature-grid">{featured.slice(0, 5).map((p, i) => <RebuildProductCard key={p.id} product={p} feature={i === 0} />)}</div></section>
    <section className="rb-statement"><span className="rb-kicker">05 / NOTRE MANIÈRE</span><h2>Pas plus de produits.<br /><em>De meilleurs choix.</em></h2><Button href="/diagnostic" light>Parler à un pharmacien <span>↗</span></Button></section>
    <section className="rb-section rb-reading"><div className="rb-section-heading"><div><span className="rb-kicker">06 / LE JOURNAL</span><h2>Lire avant<br /><em>de choisir.</em></h2></div><Link href="/journal" className="rb-text-link">Tout le journal <span>↗</span></Link></div><div className="rb-reading-grid">{posts.map((a, i) => <Link href={`/journal/${a.slug}`} className={`rb-reading-card rb-reading-${i + 1}`} key={a.id}>{a.image && <div className="rb-reading-image"><Image src={a.image} alt="" fill sizes="(max-width: 700px) 100vw, 33vw" className="object-cover" /></div>}<span>{a.tag ?? "CONSEIL"} / {a.readMinutes} MIN</span><h3>{a.title}</h3><p>{a.excerpt}</p><b>Lire l'article ↗</b></Link>)}</div><div className="rb-brands">{brandRows.map((b) => <Link href={`/marque/${b.slug}`} key={b.id}>{b.name}</Link>)}</div></section>
    {novelties.length > 0 && <section className="rb-section rb-arrivals"><div className="rb-section-heading"><div><span className="rb-kicker">07 / ARRIVAGES</span><h2>Les nouveaux<br /><em>gestes.</em></h2></div><Link href="/boutique?sort=newest" className="rb-text-link">Découvrir <span>↗</span></Link></div><RebuildProductGrid products={novelties} /></section>}
    <section className="rb-section rb-guidance"><div className="rb-section-heading"><div><span className="rb-kicker">08 / LE BON DÉPART</span><h2>Une porte pour<br /><em>chaque besoin.</em></h2></div><div><p>Vous ne savez pas par où commencer ? Nous avons prévu plusieurs façons d'entrer dans la maison.</p><Link href="/aide" className="rb-text-link">Parler au comptoir <span>↗</span></Link></div></div><div className="rb-guidance-grid"><Link href="/diagnostic" className="rb-guidance-card"><span>01 / SUR MESURE</span><h3>Faire le diagnostic</h3><p>Quelques questions, une routine plus juste et des produits expliqués.</p><b>Commencer ↗</b></Link><Link href="/boutiques" className="rb-guidance-card"><span>02 / EN PERSONNE</span><h3>Venir au comptoir</h3><p>Deux adresses, le même regard de pharmacien et le temps de vous écouter.</p><b>Voir les adresses ↗</b></Link><Link href="/livraison" className="rb-guidance-card"><span>03 / SANS DÉTOUR</span><h3>Commander sereinement</h3><p>Livraison partout en Tunisie, retrait en deux heures et suivi clair.</p><b>Comprendre la livraison ↗</b></Link></div></section>
  </div>;
}
