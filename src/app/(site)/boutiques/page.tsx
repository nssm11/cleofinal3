...
export default async function BoutiquesPage() {
  ...
  return (
    <div>
      ...
      {/* ── La salle d'accueil ─────────────────────────────────────── */}
      <section className="relative isolate overflow-hidden bg-night text-porcelain">
        <Image
        ...
        />
        <div aria-hidden className="absolute inset-0 bg-gradient-to-t from-night via-night/70 to-night/40" />
        ...
        <div className="relative container-wide pb-14 pt-28 lg:pb-20 lg:pt-36">
          ...
          <p className="rule-label mb-7 text-cinabre-3/80">La maison</p>
          <Reveal y={14} amount={0.1}>
            <h1 className="max-w-[24ch] font-display text-[clamp(2.3rem,5.2vw,4.2rem)] leading-[0.98] tracking-[-0.028em]">
              Venez rencontrer
              <span className="italic text-cinabre-3"> vos pharmaciens.</span>
            </h1>
            <p className="mt-7 max-w-[42rem] text-[15px] leading-[1.85] text-porcelain/70">
            ...
            <ul className="mt-9 flex flex-wrap gap-x-9 gap-y-3 border-t border-porcelain/15 pt-6 text-[11px] font-bold uppercase tracking-[0.18em] text-porcelain/45">
              <li className="flex items-center gap-2.5">
                <TruckIcon size={14} className="text-cinabre-3" /> Expédition partout en Tunisie
              </li>
              <li className="flex items-center gap-2.5">
                <StoreIcon size={14} className="text-cinabre-3" /> Retrait en 2 h
              </li>
            </ul>
          </Reveal>
        </div>
            ...
            <Reveal key={s.id} y={14} delay={i * 0.08} className="lg:col-span-6">
              <article className="flex h-full flex-col border-t border-ink pt-6">
                <div className="flex items-baseline gap-5">
                  <span className="font-display text-[clamp(1.6rem,2.6vw,2.2rem)] italic leading-none text-cinabre-2">
                ...
                <dl className="mt-8 flex-1 space-y-6">
                  {[
                    ...
                    <div key={row.t} className="grid grid-cols-[18px_1fr] gap-4 border-b border-rule/60 pb-5">
                      <row.i size={15} className="mt-0.5 text-cinabre-2" />
                      <div>
                        <dt className="text-[9.5px] font-bold uppercase tracking-[0.22em] text-ash">{row.t}</dt>
                        <dd className="mt-1.5 text-[14px] leading-relaxed text-slate">{row.v}</dd>
                      </div>
                    </div>
                  ))}
                  <div className="grid grid-cols-[18px_1fr] gap-4">
                    <PhoneIcon size={15} className="mt-0.5 text-cinabre-2" />
                    <div>
                      <dt className="text-[9.5px] font-bold uppercase tracking-[0.22em] text-ash">Téléphone</dt>
                      <dd className="mt-1.5">
                        <a
      ...
      {/* ── Les trois services ─────────────────────────────────────── */}
      <section className="relative overflow-hidden border-t border-rule/70 bg-bone">
        ...
        <div className="relative container-wide grid gap-px bg-rule-strong/20 sm:grid-cols-3">
          {[
            ...
            <div key={x.n} className="bg-bone px-7 py-10 lg:px-9">
              <p className="font-display text-[clamp(1.5rem,2.4vw,2rem)] italic leading-none text-cinabre-2">{x.n}</p>
              <h2 className="mt-4 text-[11px] font-bold uppercase tracking-[0.2em] text-ink">{x.t}</h2>
              <p className="mt-2.5 text-[13.5px] leading-relaxed text-graphite">{x.d}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <div className="container-wide flex flex-col items-start gap-6 py-rhythm sm:flex-row lg:py-rhythm-lg sm:items-center sm:justify-between">
        <p className="max-w-md text-[13.5px] leading-relaxed text-graphite">
