...
export default async function ComparerPage({ searchParams }: { searchParams: Promise<{ p?: string }> }) {
  ...
  return (
    <div>
      <section className="relative overflow-hidden bg-canvas pb-14 pt-28 lg:pb-16 lg:pt-36">
        <MotifLayer motif="clarity" mark={[20, 10]} />
        <div className="relative shell-wide">
          <p className="kicker-xs mb-6 flex items-center gap-3">
            <CompareIcon size={15} className="text-iodine-deep" /> Le comptoir, deux ou trois fois
          </p>
          <Reveal y={12} amount={0.1}>
            <h1 className="font-ant uppercase text-[clamp(2.1rem,4.6vw,3.6rem)] leading-[0.98] tracking-[-0.028em] text-carbon">{mm.compareTitle}</h1>
            <p className="mt-5 max-w-[52ch] text-[15px] leading-[1.9] text-muted">{mm.compareDesc}</p>
          </Reveal>
        </div>
      </section>
        ...
        <section className="shell-wide pb-24">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[560px] border-collapse text-left">
              ...
              <thead>
                <tr>
                  <th scope="col" className="w-36 border-b border-line/70 pb-4 align-bottom">
                    <span className="text-[10px] font-bold uppercase tracking-[0.22em] text-faint">—</span>
                  </th>
                  {rows.map(({ product, brandName }) => (
                    <th key={product.id} scope="col" className="border-b border-line/70 p-0 pb-4 align-bottom font-normal">
                      <div className="relative flex justify-end">
                        <Link
                          href={without(product.id)}
                          className="flex h-8 w-8 items-center justify-center text-faint transition-colors hover:text-crit"
                      ...
                      <Link href={`/produit/${product.slug}`} className="group block">
                        <span className="relative mb-3 block aspect-[4/5] w-full overflow-hidden bg-canvas-2">
                          {product.image && (
                            <Image
                              src={product.image}
                              alt=""
                        ...
                        {product.isCounterPick && (
                          <span className="mb-1 flex items-center gap-2 text-[8.5px] font-bold uppercase tracking-[0.2em] text-iodine-deep">
                            <span aria-hidden className="h-px w-3 bg-iodine" />
                            {mm.counterPick}
                          </span>
                        )}
                        {brandName && <span className="block text-[9.5px] font-bold uppercase tracking-[0.2em] text-faint">{brandName}</span>}
                        <span className="mt-1 block font-ant uppercase text-[17px] leading-snug text-carbon transition-colors group-hover:text-iodine-deep">{product.name}</span>
                      </Link>
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="align-top">
                {(
                  ...
                  <tr key={label}>
                    <th scope="row" className="border-b border-line/60 py-4 pr-4 align-top">
                      <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-faint">{label}</span>
                    </th>
                    {rows.map((row) => (
                      <td key={row.product.id} className="border-b border-line/60 px-4 py-4 text-[13.5px] leading-relaxed text-carbon">
                        {get(row) || <span className="text-faint">—</span>}
                      </td>
                    ))}
                  </tr>
                ))}
                <tr>
                  <th scope="row" className="border-b border-line/60 py-4 pr-4 align-top">
                    <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-faint">{mm.cu.price}</span>
                  </th>
                  {rows.map(({ product }) => (
                    <td key={product.id} className="border-b border-line/60 px-4 py-4">
                      <span className="font-ant uppercase text-[19px] tabular-nums text-carbon">{formatDT(product.priceMillimes)}</span>
                      {product.volume && <span className="block text-[11px] text-faint">{product.volume}</span>}
                    </td>
                  ))}
                </tr>
                <tr>
                  <th scope="row" className="py-4 pr-4 align-top">
                    <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-faint">{mm.cu.per}</span>
                  </th>
                  {rows.map(({ product }) => {
                    ...
                    return (
                      <td key={product.id} className="px-4 py-4 text-[13px] tabular-nums text-carbon">
                        {u ? u.text : <span className="text-faint">—</span>}
                      </td>
                    );
                  })}
                </tr>
