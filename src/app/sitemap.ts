...
import { listActives } from "@/lib/actives";

export const dynamic = "force-dynamic";
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const [p, c, b, k, a, act] = await Promise.all([
  ...
  const statics = [
    ...
    "/actifs",
    "/rapidite",
  ].map((path) => ({ url: `${SITE_URL}${path}`, changeFrequency: "weekly" as const, priority: path === "" ? 1 : 0.6 }));
  return [
    ...statics,
    ...
    ...act.map((x) => ({ url: `${SITE_URL}/actifs/${x.slug}`, changeFrequency: "monthly" as const, priority: 0.5 })),
  ];
}
