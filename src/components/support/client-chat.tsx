...
export function ClientChat(props: Props) {
  ...
  const thread = (
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      {conn === "reconnecting" && (
        <div className="border-b border-amber/25 bg-amber-wash/60 px-4 py-1.5 text-center text-[11px] text-amber">{live.reconnecting}</div>
      )}

      <div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto bg-canvas px-3 py-4 sm:px-5">
        {activeThread ? (
          <div className="mx-auto max-w-[44rem]">
            {activeThread.hasMore && (
              <div className="mb-2 flex justify-center">
                <button onClick={() => void loadOlder()} disabled={activeThread.loadingOlder} className="border border-line/70 bg-canvas px-4 py-1.5 text-[11px] font-bold uppercase tracking-[0.12em] text-muted transition-colors hover:border-iodine hover:text-iodine disabled:opacity-50">
          ...
          <div className="flex h-full items-center justify-center">
            <p className="text-[12.5px] text-faint">{live.messagePh}</p>
          </div>
        )}
      </div>

      {active && active.status === "resolved" && active.rating == null && (
        <div className="border-t border-line/60 bg-iodine-wash/40 px-4 py-3 sm:px-6">
          <div className="mx-auto flex max-w-[44rem] flex-wrap items-center justify-between gap-3">
            <p className="text-[12.5px] text-steel">{live.rateTitle}</p>
            <RateStars value={active.rating} onRate={(n) => void rate(n)} />
          </div>
        </div>
      )}
      {active && active.status === "resolved" && active.rating != null && (
        <div className="border-t border-line/60 bg-porcelain/50 px-4 py-2 text-center text-[11px] text-faint sm:px-6">{live.rateThanks}</div>
      )}

      <div className="border-t border-line/60 bg-canvas px-4 py-3 sm:px-5">
        <div className="mx-auto max-w-[44rem]">
          {active?.status === "closed" && <p className="mb-2 text-[11px] italic text-amber">{live.reopenNote}</p>}
          {activeId == null && (
          ...
          {orderNumber && activeId == null && (
            <p className="mb-2 flex items-center gap-1.5 text-[11px] text-iodine">
              <PackageIcon size={12} /> {orderNumber}
            </p>
          )}
          {pendingAtt && (
            <div className="mb-2">
              <span className="relative inline-flex max-w-full items-center gap-2 border border-line/70 bg-porcelain/70 px-2.5 py-1.5">
                <span className="flex h-7 w-7 shrink-0 items-center justify-center overflow-hidden border border-line/60 bg-canvas text-iodine">
                ...
                <span className="max-w-[10rem] truncate text-[11.5px] text-steel sm:max-w-[16rem]">{pendingAtt.meta.name}</span>
                <button type="button" onClick={() => setPendingAtt(null)} disabled={sending || uploading} aria-label="Retirer" className="text-faint transition-colors hover:text-crit disabled:opacity-40">
                  <CloseIcon size={12} />
                </button>
              </span>
            </div>
          ...
          <div className="flex items-end gap-2">
            ...
            <button
              ...
              className="flex h-11 w-11 shrink-0 items-center justify-center border border-line/70 text-muted transition-colors hover:border-iodine hover:text-iodine disabled:opacity-50"
            >
              <PlusIcon size={16} />
            </button>
            <textarea
            ...
            <button
              ...
              className="flex h-11 w-11 shrink-0 items-center justify-center bg-carbon text-chalk transition-colors hover:bg-iodine disabled:opacity-40"
            >
              <SendIcon size={15} className="rtl-mirror" />
            </button>
          </div>
          <p className="mt-1.5 ps-12 text-[10px] text-faint/70">
            {live.attachmentHint}
          </p>
        </div>
      </div>
  ...
  const list = (
    <div className="flex min-h-0 flex-col">
      <div className="flex items-center justify-between gap-3 border-b border-line/60 px-4 py-3">
        <p className="font-sans text-[16px] text-carbon">{copy.chat.title}</p>
        {tickets.length > 0 && <span className="text-[10px] font-bold uppercase tracking-[0.14em] tabular-nums text-faint">{tickets.length}</span>}
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto">
        {shownTickets.length === 0 ? (
          <div className="flex flex-col items-center gap-3 px-6 py-10 text-center">
            <span className="flex h-12 w-12 items-center justify-center rounded-full border border-iodine/40 bg-iodine-wash/60 text-iodine">
              <ChatIcon size={19} />
            </span>
      ...
      <div className="border-t border-line/60 bg-porcelain/50 px-4 py-3">
        <button onClick={() => { setActiveId(null); setMobileView("chat"); setDraft(""); setSubject(""); setPendingAtt(null); }} className="flex w-full items-center justify-center gap-2 border border-line/70 bg-canvas px-4 py-2.5 text-[11px] font-bold uppercase tracking-[0.16em] text-muted transition-colors hover:border-iodine hover:text-iodine">
  ...
  const presenceLine = (
    <p className="flex items-center gap-2.5 text-[12px] text-muted">
      <span className="relative flex h-2 w-2">
        {online && <span aria-hidden className="absolute inline-flex h-full w-full animate-ping rounded-full bg-ok opacity-50 motion-reduce:animate-none" />}
        <span className={cn("relative inline-flex h-2 w-2 rounded-full", online ? "bg-ok" : "bg-canvas-2")} />
      </span>
  ...
  return (
    <div>
      ...
      <div className="flex h-[66dvh] max-h-[52rem] min-h-[30rem] overflow-hidden rounded-[3px] border border-line/70 bg-canvas shadow-soft">
        <div className={cn("w-full flex-col border-e border-line/70 md:flex md:w-[290px] md:shrink-0 lg:w-[320px]", mobileView === "list" ? "flex" : "hidden")}>{list}</div>
        <div className={cn("min-w-0 flex-1 flex-col md:flex", mobileView === "chat" ? "flex" : "hidden")}>
          {active ? thread : (
            <div className="flex flex-1 flex-col items-center justify-center gap-3 bg-canvas p-8 text-center">
              <ChatAvatar name={copy.chat.staffName} maison size={52} />
              <p className="max-w-[18rem] font-sans text-[17px] leading-snug text-carbon">{copy.chat.title}</p>
              <p className="max-w-[18rem] text-[12.5px] leading-relaxed text-faint">{copy.chat.openHours}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="pointer-events-none fixed bottom-5 start-1/2 z-50 flex w-[min(24rem,90vw)] -translate-x-1/2 flex-col gap-2 rtl:translate-x-1/2">
        {toasts.map((t) => (
          <div key={t.id} className="pointer-events-auto border border-carbon bg-carbon px-4 py-2.5 text-[12px] leading-relaxed text-chalk shadow-float">
            {t.text}
          </div>
        ))}
      </div>
