...
export function StatusButtons({ orderId, status }: { orderId: number; status: OrderStatus }) {
  ...
  if (!nexts.length) return <p className="text-sm text-ops-muted">Commande clôturée.</p>;
  return (
...
export function PaymentControl({ orderId, method, status, isPaid }: { orderId: number; method: string; status: string; isPaid: boolean }) {
  const [pending, start] = useTransition();
  const { toast } = useToast();
  if (method === "cod") return <p className="mt-1 text-xs text-ops-muted">COD — le statut se règle tout seul à la livraison.</p>;
  ...
  return (
    <div className="mt-2 flex flex-wrap items-center gap-2">
      <span className={`px-2 py-0.5 text-[9px] font-bold uppercase tracking-[0.14em] ${isPaid ? "bg-ok-wash text-ok" : "bg-ops-sheet-2 text-ops-muted"}`}>{PAY_LABEL[status] ?? status}</span>
      {!isPaid && status !== "refunded" && <button disabled={pending} onClick={() => act("paid", "Encaissement enregistré")} className="min-h-9 border border-ops-signal/40 px-2.5 text-[9px] font-bold uppercase tracking-[0.14em] text-ops-signal hover:bg-ops-signal/10 disabled:opacity-40">Marquer payé</button>}
      {isPaid && <button disabled={pending} onClick={() => act("refunded", "Marqué remboursé")} className="min-h-9 border border-ops-line px-2.5 text-[9px] font-bold uppercase tracking-[0.14em] text-ops-muted hover:text-ops-ink disabled:opacity-40">Marquer remboursé</button>}
      {(status === "refunded" || status === "failed") && <button disabled={pending} onClick={() => act("pending", "Retour en attente")} className="min-h-9 border border-ops-line px-2.5 text-[9px] font-bold uppercase tracking-[0.14em] text-ops-muted hover:text-ops-ink disabled:opacity-40">Repasser en attente</button>}
    </div>
  );
}
...
export function BulkBar({ ids, onDone }: { ids: number[]; onDone: () => void }) {
  ...
  return (<div className="mb-3 flex flex-wrap items-center gap-2 border border-ops-line bg-ops-sheet p-3 text-sm"><span className="text-ops-muted">{ids.length} sélectionnée(s) →</span>{(["confirmed", "preparing", "shipped", "delivered"] as const).map((s) => <button key={s} disabled={pending} onClick={() => start(async () => { const r = await bulkOrderStatusAction(ids, s); toast({ kind: "info", title: r.ok ? r.message ?? "" : r.error }); onDone(); router.refresh(); })} className={abtnGhost}>{ORDER_STATUS_LABELS[s]}</button>)}</div>);
}
