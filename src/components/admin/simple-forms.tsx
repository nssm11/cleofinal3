...
export function PromotionForm({ promo, universes, onDone }: { promo?: Promotion; universes: { id: number; name: string }[]; onDone?: () => void }) {
  ...
  return (
    <form action={action} className="grid gap-3 border border-ops-line bg-ops-sheet p-4 sm:grid-cols-4">{promo && <input type="hidden" name="id" value={promo.id} />}
      ...
      <label className="flex min-h-11 items-center gap-2 self-end text-sm"><input type="checkbox" name="isActive" defaultChecked={promo?.isActive ?? true} className="h-4 w-4 accent-iodine" /> Active</label>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">{promos.map((p) => editing === p.id ? <PromotionForm key={p.id} promo={p} universes={universes} onDone={() => setEditing(null)} /> : (
        <div key={p.id} className="flex flex-wrap items-center justify-between gap-3 border border-ops-line px-4 py-3 text-sm"><div><span className="font-mono">{p.code}</span><span className="ml-3 text-ops-muted">{p.label}</span></div><div className="flex items-center gap-4 text-xs text-ops-muted"><span>{p.usageCount}{p.usageLimit ? `/${p.usageLimit}` : ""} utilisations</span><span className={p.isActive ? "text-ok" : "text-crit"}>{p.isActive ? "Active" : "Inactive"}</span><button onClick={() => setEditing(p.id)} className="text-ops-ink hover:underline">Modifier</button><DeleteSlot id={p.id} /></div></div>
      ))}</div>
    </div>
  );
}
...
export function ArticleForm({ article, onDone }: { article?: Article; onDone?: () => void }) {
  ...
  useFb(state, onDone);
  return (
    <form action={action} className="grid gap-3 border border-ops-line bg-ops-sheet p-4">{article && <input type="hidden" name="id" value={article.id} />}
      ...
      <label className="flex min-h-11 items-center gap-2 text-sm"><input type="checkbox" name="isPublished" defaultChecked={article?.isPublished ?? true} className="h-4 w-4 accent-iodine" /> Publié</label>
...
export function ArticlesManager({ list }: { list: Article[] }) {
  const [editing, setEditing] = useState<number | "new" | null>(null);
  return (<div className="space-y-6">{editing === "new" ? <ArticleForm onDone={() => setEditing(null)} /> : <button onClick={() => setEditing("new")} className={abtn}>Nouvel article</button>}<div className="space-y-2">{list.map((a) => editing === a.id ? <ArticleForm key={a.id} article={a} onDone={() => setEditing(null)} /> : <div key={a.id} className="flex flex-wrap items-center justify-between gap-3 border border-ops-line px-4 py-3 text-sm"><div>{a.title}<span className="ml-3 text-xs text-ops-muted">{a.tag} · /journal/{a.slug}</span></div><div className="flex items-center gap-4 text-xs"><span className={a.isPublished ? "text-ok" : "text-ops-muted"}>{a.isPublished ? "Publié" : "Brouillon"}</span><button onClick={() => setEditing(a.id)} className="hover:underline">Modifier</button></div></div>)}</div></div>);
}
export function StoreForm({ store, onDone }: { store?: Store; onDone?: () => void }) {
  ...
  useFb(state, onDone);
  return (
    <form action={action} className="grid gap-3 border border-ops-line bg-ops-sheet p-4 sm:grid-cols-3">{store && <input type="hidden" name="id" value={store.id} />}
      ...
      <label className="flex min-h-11 items-center gap-2 self-end text-sm"><input type="checkbox" name="isActive" defaultChecked={store?.isActive ?? true} className="h-4 w-4 accent-iodine" /> Active</label>
      <div className="flex items-end gap-2"><button disabled={pending} className={abtn}>Enregistrer</button>{onDone && <button type="button" onClick={onDone} className={abtnGhost}>Annuler</button>}</div>
    </form>
  );
}
export function StoresManager({ list }: { list: Store[] }) {
  const [editing, setEditing] = useState<number | "new" | null>(null);
  return (<div className="space-y-6">{editing === "new" ? <StoreForm onDone={() => setEditing(null)} /> : <button onClick={() => setEditing("new")} className={abtn}>Nouvelle boutique</button>}<div className="space-y-2">{list.map((s) => editing === s.id ? <StoreForm key={s.id} store={s} onDone={() => setEditing(null)} /> : <div key={s.id} className="flex flex-wrap items-center justify-between gap-3 border border-ops-line px-4 py-3 text-sm"><div>{s.name}<span className="ml-3 text-xs text-ops-muted">{s.address} · {s.phone}</span></div><div className="flex items-center gap-4 text-xs"><span className={s.isActive ? "text-ok" : "text-ops-muted"}>{s.isActive ? "Active" : "Inactive"}</span><button onClick={() => setEditing(s.id)} className="hover:underline">Modifier</button></div></div>)}</div></div>);
}
