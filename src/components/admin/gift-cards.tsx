  ...
  return (
    <div className="grid gap-6 lg:grid-cols-12">
      <div className="lg:col-span-4">
        <Panel title="Émettre une carte">
          <div className="space-y-4 p-5">
            <div>
              <label htmlFor="gc-amount" className="mb-1.5 block text-[11px] font-bold uppercase tracking-[0.16em] text-ops-muted">Montant (DT)</label>
              <input
                id="gc-amount"
                type="number"
                min={1}
                ...
                className="w-full border border-ops-line bg-os-bg px-3 py-2.5 text-ops-ink"
              />
            </div>
            <div>
              <label htmlFor="gc-exp" className="mb-1.5 block text-[11px] font-bold uppercase tracking-[0.16em] text-ops-muted">Expiration (facultatif)</label>
              <input
                id="gc-exp"
                type="date"
                value={expiresAt}
            ...
            <div>
              <label htmlFor="gc-note" className="mb-1.5 block text-[11px] font-bold uppercase tracking-[0.16em] text-ops-muted">Note (facultatif)</label>
              <input
                id="gc-note"
                value={note}
            ...
            <button onClick={issue} disabled={pending} className="w-full bg-ops-signal px-4 py-3 text-[12px] font-bold uppercase tracking-[0.16em] text-os-carbon transition-opacity disabled:opacity-50">
            ...
            {issued && (
              <div className="border border-ops-signal/50 bg-ops-signal/10 p-4" role="status">
                <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-ops-signal">Carte émise — {dt(issued.amountMillimes)}</p>
                <p className="mt-2 select-all font-mono text-lg tracking-wide text-ops-ink">{issued.code}</p>
                <p className="mt-2 text-[12px] text-ops-muted">Remettez ce code au client maintenant : il ne sera plus jamais affiché.</p>
              </div>
            )}
          </div>
        </Panel>
      ...
      <div className="lg:col-span-8">
        ...
        >
          {cards.length === 0 ? (
            <p className="p-8 text-center text-[13px] text-ops-muted">Aucune carte émise pour l’instant.</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-left text-[13px]">
                <thead>
                  <tr className="border-b border-ops-line text-[10px] uppercase tracking-[0.16em] text-ops-muted">
                ...
                <tbody className="divide-y divide-ops-line">
                  {cards.map((c) => {
                    ...
                    return (
                      <tr key={c.id}>
                        <td className="px-4 py-3 font-mono text-ops-ink">…{c.codePrefix}</td>
                        <td className="px-4 py-3 tabular-nums text-ops-ink">{dt(c.initialMillimes)}</td>
                        <td className="px-4 py-3 tabular-nums text-ops-ink">{dt(c.balanceMillimes)}</td>
                        <td className="px-4 py-3">
                          <span className={`inline-block px-2 py-0.5 text-[11px] font-bold uppercase tracking-[0.12em] ${live === "active" ? "bg-emerald-500/15 text-emerald-600" : live === "redeemed" ? "bg-ops-signal/15 text-ops-signal" : live === "expired" ? "bg-os-danger/10 text-os-danger" : "bg-ops-sheet-2 text-ops-muted"}`}>
                            {live === "active" ? "Active" : live === "redeemed" ? "Épuisée" : live === "expired" ? "Expirée" : "Annulée"}
                          </span>
                        </td>
                        <td className="px-4 py-3 text-ops-muted">{c.expiresAt ? new Date(c.expiresAt).toLocaleDateString("fr-TN") : "—"}</td>
                        <td className="max-w-[12rem] truncate px-4 py-3 text-ops-muted">{c.note ?? "—"}</td>
