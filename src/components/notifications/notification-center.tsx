  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center gap-2" role="tablist" aria-label={t.title}>
        ...
        <span className="mx-1 hidden h-5 w-px bg-canvas-2/60 sm:block" aria-hidden />
        <button
          ...
          className={cn(
            ...
            unreadOnly ? "border-carbon bg-carbon text-chalk" : "border-line/70 bg-canvas/70 text-muted hover:text-carbon",
          )}
        >
          <CheckIcon size={12} />
          {t.unreadOnly}
        ...
        {unreadTotal > 0 && (
          <button
            onClick={markAll}
            className="ms-auto text-[10.5px] font-bold uppercase tracking-[0.16em] text-iodine transition-colors hover:text-carbon"
          >
            {t.markAll}
          </button>
        )}
      ...
      {/* ── The shelf ───────────────────────────────────────────────── */}
      <AccountCard className="mt-6 !border-line/50 !bg-transparent !shadow-none" hover={false}>
        {loading ? (
          ...
          <>
            ...
            {hasMore && (
              <div className="border-t border-line/60 px-6 py-5 text-center">
...
function FilterChip({
  ...
  return (
    <button
      ...
      className={cn(
        "flex items-center gap-2 rounded-full border px-4 py-2 text-[10.5px] font-bold uppercase tracking-[0.14em] transition-colors duration-300",
        active ? "border-iodine/70 bg-iodine-wash/80 text-carbon" : "border-line/60 bg-canvas/70 text-muted hover:text-carbon",
      )}
    >
      {icon}
      {label}
      {count > 0 && (
        <span
          className={cn(
            ...
            active ? "bg-carbon text-chalk" : "bg-iodine/20 text-iodine",
          )}
        >
          {count > 99 ? "99+" : count}
        </span>
