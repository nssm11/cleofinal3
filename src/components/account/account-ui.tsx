...
export function AccountCard({
  ...
  return (
    <Tag
      className={cn(
        "relative overflow-hidden rounded-[3px] border border-rule/60 bg-alabaster shadow-whisper",
        "transition-[transform,box-shadow,border-color] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]",
        hover && "hover:-translate-y-[3px] hover:border-rule-strong/70 hover:shadow-soft",
        className,
      )}
    >
      {accent && (
        <span
          aria-hidden
          className="absolute inset-x-0 top-0 h-[2px] bg-gradient-to-r from-cinabre-2/0 via-cinabre-2 to-cinabre-2/0"
        />
      )}
      {children}
    </Tag>
  ...
  return (
    <div className="space-y-4" aria-hidden>
      {Array.from({ length: n }).map((_, i) => (
        <div key={i} className={cn("rounded-[3px] border border-rule/60 bg-alabaster p-6", tall && "p-8")}>
...
export function StatRowSkeleton({ n = 3 }: { n?: number }) {
  return (
    <div className="grid gap-4 sm:grid-cols-3" aria-hidden>
      {Array.from({ length: n }).map((_, i) => (
        <div key={i} className="rounded-[3px] border border-rule/60 bg-alabaster p-7">
