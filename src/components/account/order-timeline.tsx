    ...
    <div>
      ...
      {(paymentMethod || trackingCode) && !terminal && (
        <div className="mb-10 grid gap-3 sm:grid-cols-2">
          {paymentMethod && (
            <div className="flex items-center gap-3.5 border border-line/60 bg-mist/60 px-4 py-3.5">
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-iodine-deep/40 bg-iodine-wash/60 text-iodine-deep">
                <CardIcon size={15} />
              </span>
              <span className="min-w-0">
                <span className="block text-[9.5px] font-bold uppercase tracking-[0.2em] text-faint">
                  {copy.tracking.paymentBlock}
                </span>
                <span className="mt-0.5 block truncate text-[13.5px] text-carbon">
                  {PAYMENT_LABELS[paymentMethod]}
                  <span
                    className={`ms-2 text-[10px] font-bold uppercase tracking-[0.14em] ${ paymentStatus === "paid" ? "text-ok" : paymentStatus === "refunded" ? "text-iodine-deep" : "text-faint" }`}
                  >
                    · {payLabel}
                  </span>
                </span>
          ...
          {trackingCode && (
            <div className="flex items-center gap-3.5 border border-line/60 bg-mist/60 px-4 py-3.5">
              ...
              <span className="min-w-0 flex-1">
                ...
                <span className="mt-0.5 block truncate font-mono text-[13px] text-carbon">{trackingCode}</span>
              </span>
              <a
                ...
                className="flex h-9 w-9 shrink-0 items-center justify-center border border-line/60 text-muted transition-colors hover:border-iodine-deep hover:text-iodine-deep"
              >
      ...
      {events.length > 0 && (
        <details className="group mt-10 border border-line/60 bg-mist/40">
          <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-4 text-[11px] font-bold uppercase tracking-[0.2em] text-muted transition-colors hover:text-carbon [&::-webkit-details-marker]:hidden">
            {journalLabel}
            <span className="flex items-center gap-2">
              <Seal kind={terminal ? "error" : "gold"}>{events.length}</Seal>
              <span aria-hidden className="text-iodine-deep transition-transform duration-300 group-open:rotate-180">▾</span>
            </span>
          </summary>
          <ul className="space-y-4 border-t border-line/60 px-5 py-5">
            {events.map((e, i) => (
              <li key={i} className="relative border-s-2 border-iodine/60 ps-4 text-sm">
                <p className="text-carbon">
                  {st[e.status] ?? e.status}
                  {e.message ? <span className="text-muted"> — {e.message}</span> : null}
                </p>
                <p className="mt-0.5 text-xs tabular-nums text-faint">{formatDateTime(e.createdAt)}</p>
              </li>
            ))}
          </ul>
        </details>
      ...
      {status === "delivered" && (
        <p className="mt-6 border-t border-line/70 pt-5 text-[12.5px] leading-relaxed text-muted">{copy.tracking.deliveredNote}</p>
      )}

      {orderNumber && (
        <div className="mt-8 flex flex-wrap items-center gap-x-5 gap-y-3 border-t border-line/70 pt-6">
          <p className="inline-flex items-center gap-2 text-[12px] text-muted">
            <WarningIcon size={13} className="text-iodine-deep" /> {copy.tracking.problem} ?
          </p>
