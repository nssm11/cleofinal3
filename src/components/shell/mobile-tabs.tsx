...
export function MobileTabs({ onSearch }: { onSearch: () => void }) {
  ...
  return (
    ...
    >
      <div className="mx-3 mb-3 flex items-stretch border border-stone-2/30 bg-cream/90 px-1 shadow-float backdrop-blur-2xl">
        {ITEMS.map((item) => {
            ...
            >
              {active && !reduce && (
                <motion.span
                  ...
                  className="absolute inset-x-3 top-0 h-px bg-champagne-2"
                />
              )}
              {active && reduce && <span aria-hidden className="absolute inset-x-3 top-0 h-px bg-champagne-2" />}
              <Icon size={19} className={cn("transition-colors duration-300", active ? "text-ink" : "text-muted-2")} />
              <span
                className={cn(
        ...
        <button
          ...
          className="relative flex min-h-[58px] flex-1 flex-col items-center justify-center gap-1 px-1"
        >
          <SearchIcon size={19} className="text-muted-2" />
          <span className="text-[9px] font-bold uppercase tracking-[0.14em] text-muted-2">Chercher</span>
        </button>
      </div>
    </nav>
  );
