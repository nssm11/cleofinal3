          ...
          {chapters.map((c) => {
            ...
            return (
              <li key={c.slug} className="w-64 shrink-0 snap-start sm:w-80 lg:w-auto">
                <Link
                  href={`/univers/${c.slug}`}
                  className="group relative block aspect-[3/4] overflow-hidden bg-cine-noir-2 lg:aspect-[4/5]"
                >
                  {c.image && (
                  ...
                  <div aria-hidden className="absolute inset-0 bg-gradient-to-t from-cine-noir/85 via-cine-noir/20 to-transparent" />
                  <div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-3 p-5">
                    <div className="min-w-0">
                      <p className="cine-kicker text-cine-gold!">{cinema?.kicker ?? copy.univers.label}</p>
                      <p className="mt-2 truncate font-display text-[24px] font-light leading-tight text-cine-ivory">{c.name}</p>
                    </div>
                    <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-cine-ivory/30 text-cine-ivory backdrop-blur-sm transition-all duration-500 group-hover:border-cine-gold group-hover:bg-cine-gold group-hover:text-cine-noir">
