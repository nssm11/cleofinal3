...
export function VisageRayons({
  ...
  return (
    <section id="rayons" aria-label={fmt(copy.univers.seeCategories, { n: rayons.length })} className="scroll-mt-16 bg-cine-noir">
          ...
          <div className="lg:sticky lg:top-28">
            ...
            {image && (
              <div className="relative mt-10 hidden aspect-[4/5] overflow-hidden lg:block">
                ...
                <div aria-hidden className="absolute inset-0 bg-gradient-to-t from-cine-noir/60 via-transparent to-transparent" />
        ...
        <ul className="lg:col-span-7">
          {image && (
            <li aria-hidden className="relative mb-10 aspect-[16/9] overflow-hidden lg:hidden">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-cine-noir/60 via-transparent to-transparent" />
            </li>
          )}
          {rayons.map((c, i) => (
              ...
              <Link href={`/categorie/${c.slug}`} className="group grid grid-cols-[auto_1fr_auto] items-center gap-4 py-6 sm:gap-7">
                ...
                <span className="min-w-0">
                  <span className="flex flex-wrap items-baseline gap-x-4 gap-y-1">
                    <span className="font-display text-[clamp(1.35rem,2.6vw,1.9rem)] font-light leading-tight text-cine-ivory transition-colors duration-500 group-hover:text-cine-gold">
                      {c.name}
                    </span>
                    <span className="text-[10.5px] font-bold uppercase tracking-[0.2em] text-cine-faint">
                      {c.count} réf{c.count !== 1 ? "s" : ""}
                    </span>
                  </span>
                  {c.description && (
                    <span className="mt-1.5 line-clamp-2 block text-[13px] leading-relaxed text-cine-mist">{c.description}</span>
                  )}
                </span>
                <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-cine-line text-cine-mist transition-all duration-500 group-hover:border-cine-gold group-hover:bg-cine-gold group-hover:text-cine-noir">
