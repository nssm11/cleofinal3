...
export function DuoOffer({ duo, labels }: { duo: DuoOfferData; labels: { eyebrow: string; together: string; save: string; add: string; added: string } }) {
  ...
  return (
    <motion.div
      ...
      className="relative mt-10 overflow-hidden border border-cinabre/35 bg-cinabre-soft/30 p-6"
    >
      <p className="flex items-center gap-2.5 text-[9.5px] font-bold uppercase tracking-[0.24em] text-cinabre-2">
      ...
      {duo.note && <p className="mt-2.5 max-w-prose text-[13px] leading-relaxed text-graphite">{duo.note}</p>}

      <ul className="mt-5 space-y-3">
        {duo.members.map((m, i) => (
          <li key={m.id} className="flex items-center gap-3.5">
            <span className="font-display text-[13px] italic text-cinabre-2">{i === 0 ? "1" : "2"}</span>
            <span className="relative h-14 w-11 shrink-0 overflow-hidden bg-bone-2">
      ...
      <button
        onClick={add}
        className="mt-6 flex w-full items-center justify-between gap-3 border-b border-cinabre-2 pb-1.5 text-[10px] font-bold uppercase tracking-[0.2em] text-cinabre-2 transition-colors hover:text-ink"
      >
