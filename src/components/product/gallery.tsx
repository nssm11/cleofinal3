      ...
      <div className={cn("flex gap-4", many ? "flex-col sm:flex-row-reverse sm:gap-5" : "")}>
        ...
        <div className="relative flex-1">
          <div
            ...
            className={cn("relative aspect-square w-full overflow-hidden bg-canvas-2", out && "saturate-[0.4]")}
          >
            <motion.button
              type="button"
            ...
            <p className="pointer-events-none absolute inset-x-0 bottom-0 z-20 flex items-center justify-between gap-4 border-t border-line/60 bg-canvas/92 px-5 py-3 text-[9.5px] font-bold uppercase tracking-[0.22em] text-muted">
          ...
          {many && (
            <p className="mt-3 flex items-center justify-between gap-4 text-[10px] font-bold uppercase tracking-[0.2em] text-faint">
              <span>
            ...
            {list.map((src, i) => (
              <li key={`${src}-${i}`} className="shrink-0">
                <button
                  ...
                  className={cn(
                    "relative block h-[78px] w-[68px] overflow-hidden bg-canvas-2 transition-opacity duration-300 sm:h-[96px] sm:w-full",
                ...
                >
                  ...
                  <span
                    aria-hidden
                    className={cn(
                      "absolute inset-x-0 bottom-0 h-px bg-iodine transition-transform duration-500",
      ...
      <AnimatePresence>
        {full && (
          <motion.div
            ...
            className="fixed inset-0 z-[90] flex flex-col bg-carbon/96 backdrop-blur-xl"
          >
            <div className="flex items-center justify-between px-4 py-3 lg:px-8">
              <span className="text-[10px] font-bold uppercase tracking-[0.24em] text-chalk/55">
                {brandName} — {name}
              </span>
              <button
                onClick={() => setFull(false)}
                aria-label="Fermer"
                className="flex h-11 w-11 items-center justify-center text-chalk/70 transition-colors hover:text-chalk"
              >
                <CloseIcon size={22} />
              </button>
            </div>
            ...
            {many && (
              <div className="flex items-center justify-center gap-8 pb-8">
                <button
                  ...
                  className="flex h-12 w-12 items-center justify-center border border-line/25 text-chalk/80 transition-colors hover:border-line hover:text-chalk"
                >
                  <MinusIcon size={16} />
                </button>
                <span className="text-[11px] tabular-nums text-chalk/60">
                  {index + 1} / {list.length}
                </span>
                <button
                  onClick={() => step(1)}
