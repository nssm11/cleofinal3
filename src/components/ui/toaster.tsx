        ...
        <AnimatePresence initial={false}>
          {items.map((t) => (
            <motion.div
              ...
              className="pointer-events-auto flex w-full max-w-sm items-start gap-3 border border-line bg-mist px-4 py-3 shadow-lift"
              role="status"
            >
              <span className={t.kind === "error" ? "text-crit" : t.kind === "info" ? "text-muted" : "text-ok"}>
                {t.kind === "error" ? <WarningIcon size={18} /> : t.kind === "info" ? <InfoIcon size={18} /> : <CheckIcon size={18} />}
              </span>
              <div className="min-w-0 flex-1">
                <p className="text-sm font-medium text-carbon">{t.title}</p>
                {t.description && <p className="mt-0.5 text-xs text-muted">{t.description}</p>}
                {t.action && (
                  <button onClick={() => { t.action?.onClick(); dismiss(t.id); }} className="mt-2 text-xs uppercase tracking-[0.14em] text-iodine-deep underline-offset-4 hover:underline">
                    {t.action.label}
                  </button>
                )}
              </div>
              <button onClick={() => dismiss(t.id)} aria-label="Fermer" className="-m-1 flex h-8 w-8 items-center justify-center text-muted hover:text-carbon">
                <CloseIcon size={14} />
              </button>
            </motion.div>
          ))}
