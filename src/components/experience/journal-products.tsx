...
export function CartAddButton({ line }: { line: { productId: number; slug: string; name: string; brandName: string | null; image: string | null; priceMillimes: number; stock: number; volume: string | null } }) {
  ...
  return (
    <button
      ...
      className="shrink-0 border-b border-iodine-deep/60 pb-0.5 text-[10px] font-bold uppercase tracking-[0.18em] text-carbon transition-colors hover:border-iodine-deep hover:text-iodine-deep disabled:opacity-40"
    >
      {copy.product.add}
    </button>
  );
