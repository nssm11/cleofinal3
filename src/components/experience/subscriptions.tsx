    ...
    <ul className="space-y-6">
      {subs.map((s) => (
        <motion.li
          ...
          className={`border p-5 lg:p-6 ${s.status === "cancelled" ? "border-rule-strong/40 bg-porcelain/50 opacity-70" : s.status === "paused" ? "border-warning/40 bg-warning-soft/40" : "border-cinabre/40 bg-bone/70"}`}
        >
          <div className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1">
            <p className="font-display text-[20px] text-ink">
              {t.every.replace("{n}", String(s.frequencyDays))}
            </p>
            <p className={`text-[10px] font-bold uppercase tracking-[0.2em] ${s.status === "active" ? "text-success" : s.status === "paused" ? "text-warning" : "text-ash"}`}>
              {t.status[s.status]}
            </p>
          </div>
          <p className="mt-1 text-[12px] text-graphite">
            {t.nextDelivery}: <span className="text-slate">{new Date(s.nextDueAt).toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" })}</span>
            {" · "}
            {t.totalEstimate}: <span className="tabular-nums text-slate">{formatDT(s.totalEstimate)}</span> <span className="text-ash">(−5 %)</span>
          </p>

          <ul className="mt-4 divide-y divide-rule/70 border-y border-rule/70">
            {s.items.map((it) => (
              <li key={it.id} className="flex items-center gap-3.5 py-3">
                <span className="relative h-12 w-10 shrink-0 overflow-hidden bg-bone-2">
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block text-[11px] text-ash">{it.brandName}</span>
                </span>
                <span className="text-[12.5px] tabular-nums text-slate">{formatDTShort(it.priceMillimes)}</span>
                {s.status !== "cancelled" && (
                  <button
          ...
          {s.status !== "cancelled" && (
            <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2">
              <button disabled={pending} onClick={() => act(() => setSubscriptionStatusAction(s.id, s.status === "paused" ? "active" : "paused"), s.status === "paused" ? copy.common.saved : t.pausedMsg)} className="inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.18em] text-ink transition-colors hover:text-cinabre-2 disabled:opacity-40">
                {s.status === "paused" ? <PlayIcon size={12} /> : <PauseIcon size={12} />}
                {s.status === "paused" ? t.resume : t.pause}
              </button>
              <button disabled={pending} onClick={() => act(() => skipNextDeliveryAction(s.id), t.skippedMsg)} className="inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.18em] text-ink transition-colors hover:text-cinabre-2 disabled:opacity-40">
                <SkipIcon size={12} /> {t.skip}
              </button>
              <label className="inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.18em] text-graphite">
                <RefreshIcon size={12} />
                {t.changeFreq}
                <select
                  defaultValue={s.frequencyDays}
                  onChange={(e) => act(() => setSubscriptionFrequencyAction(s.id, Number(e.target.value)), t.freqDone)}
                  className="border-b border-rule-strong/60 bg-transparent py-1 text-[11px] tracking-normal text-slate focus:border-cinabre focus:outline-none"
                >
              ...
              <button disabled={pending} onClick={() => act(() => setSubscriptionStatusAction(s.id, "cancelled"), t.cancelled)} className="ms-auto inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.18em] text-graphite transition-colors hover:text-error disabled:opacity-40">
                <TrashIcon size={12} /> {t.cancel}
              </button>
            </div>
          )}
          <p className="mt-3 text-[11.5px] italic text-ash">{t.codNote}</p>
        </motion.li>
      ))}
    </ul>
  );
...
function SwapPicker({ onPick }: { onPick: (productId: number) => void }) {
  ...
  return (
    <div className="mt-3 border border-cinabre/40 bg-porcelain/80 p-3">
      <label className="block text-[9.5px] font-bold uppercase tracking-[0.2em] text-cinabre-2">{copy.subscription.swap}</label>
      <input
        value={q}
        onChange={(e) => {
          setQ(e.target.value);
      ...
      {rows.length > 0 && (
        <ul className="mt-2 max-h-56 overflow-auto">
          {rows.map((p) => (
            <li key={p.id}>
              <button type="button" onClick={() => onPick(p.id)} className="flex w-full items-center gap-3 border-b border-rule/60 py-2 text-start last:border-b-0 hover:text-cinabre-2">
                <span className="relative h-9 w-7 shrink-0 overflow-hidden bg-bone-2">
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block text-[10.5px] text-ash">{p.brandName}</span>
                </span>
                <span className="text-[11.5px] tabular-nums text-slate">{formatDTShort(p.priceMillimes)}</span>
              </button>
            </li>
          ))}
        </ul>
  ...
  return (
    <div>
      ...
      {suggestions.length > 0 && (
        <ul className="mb-4 grid gap-px border border-rule-strong/25 bg-rule-strong/20 sm:grid-cols-2">
          {suggestions.map((s) => {
            const on = picked.includes(s.id);
            return (
              <li key={s.id}>
                <button type="button" onClick={() => toggle(s.id)} aria-pressed={on} className={`flex w-full items-center gap-3 px-4 py-3 text-start transition-colors ${on ? "bg-cinabre-soft/60" : "bg-porcelain hover:bg-bone"}`}>
                  <span aria-hidden className={`flex h-4 w-4 shrink-0 items-center justify-center border ${on ? "border-cinabre-2 bg-cinabre-2 text-porcelain" : "border-rule-strong/70"}`}>{on && "✓"}</span>
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-[13px] text-ink">{s.name}</span>
                    <span className="block text-[10.5px] text-ash">{s.brandName}</span>
                  </span>
                </button>
              </li>
            );
      ...
      />
      {rows.length > 0 && (
        <ul className="mt-2 border border-rule-strong/40">
          {rows.map((p) => (
            <li key={p.id}>
              <button type="button" onClick={() => { toggle(p.id); setQ(""); setRows([]); }} className="flex w-full items-center gap-3 border-b border-rule/60 px-3 py-2 text-start last:border-b-0 hover:bg-bone">
                <span className="min-w-0 flex-1 truncate text-[12.5px] text-ink">{p.name}</span>
                <span className="text-[11px] text-ash">{formatDTShort(p.priceMillimes)}</span>
                <PlusIcon size={12} className="text-cinabre-2" />
              </button>
            </li>
          ))}
        </ul>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-4">
        <label className="flex items-center gap-3 text-[10px] font-bold uppercase tracking-[0.18em] text-graphite">
      ...
      <p className="mt-3 text-[11.5px] text-ash">{t.perks.join(" · ")}</p>
    </div>
  );
}
