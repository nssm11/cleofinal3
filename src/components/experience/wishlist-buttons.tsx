...
export function RemoveWishButton({ productId }: { productId: number }) {
  ...
  return (
    <button
      ...
      className="flex h-9 w-9 items-center justify-center text-ash transition-colors hover:text-error"
    >
      <TrashIcon size={14} />
    </button>
  );
...
export function WishToList({
  ...
  return (
    <button
      ...
      className="inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.18em] text-ink transition-colors hover:text-cinabre-2 disabled:opacity-40"
    >
...
export function WishHeart({ productId, wished: initial }: { productId: number; wished: boolean }) {
  ...
  return (
    <button
      ...
      className={`flex h-9 w-9 items-center justify-center transition-colors ${wished ? "text-cinabre" : "text-ash hover:text-ink"}`}
    >
