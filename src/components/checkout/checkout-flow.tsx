      ...
      <div className="lg:col-span-7">
        ...
        <div className="mt-12 overflow-hidden">
          <AnimatePresence mode="wait" initial={false}>
            {step === 0 && (
              <motion.section key="info" variants={variants} initial="enter" animate="center" exit="exit" className="space-y-7">
                ...
                {!user && (
                  <div className="border border-line/60 bg-mist/60 px-5 py-4"><label className="flex min-h-14 cursor-pointer items-center gap-3 text-[15.5px]"><input type="checkbox" checked={createAccount} onChange={(e) => setCreateAccount(e.target.checked)} className="h-5 w-5 accent-iodine" /> Créer un compte pour suivre mes commandes</label>{createAccount && <Field label="Mot de passe" error={errors.accountPassword}><input type="password" value={accountPassword} onChange={(e) => setAccountPassword(e.target.value)} autoComplete="new-password" className="field-box" /></Field>}</div>
                )}
              </motion.section>
            )}
            {step === 1 && (
              <motion.section key="ship" variants={variants} initial="enter" animate="center" exit="exit" className="space-y-7">
                ...
                <div className="space-y-2" role="radiogroup">
                  {([
                    ...
                    <label key={o.v} className={`flex min-h-[5.5rem] cursor-pointer items-center gap-4 border px-5 py-4 transition-colors ${shipping === o.v ? "border-iodine bg-iodine-wash/60" : "border-line/60 hover:border-line-strong"}`}>
                      <input type="radio" name="shipping" value={o.v} checked={shipping === o.v} onChange={() => setShipping(o.v)} className="sr-only" />
                      <o.i size={22} className="shrink-0 text-iodine-deep" /><div className="flex-1"><p className="text-[15.5px] text-carbon">{o.l}</p><p className="mt-0.5 text-[13px] leading-relaxed text-muted">{o.d}</p></div><span className="shrink-0 text-[15.5px] tabular-nums text-carbon">{o.p === 0 || (promo?.freeShipping && o.v === "standard") ? "Offerte" : formatDT(o.p)}</span>
                    </label>
                  ))}
                </div>
                ...
                <Field label="Note pour la commande (facultatif)"><textarea value={cart.note} onChange={(e) => cart.setNote(e.target.value)} rows={3} maxLength={500} className="field-area" /></Field>
              </motion.section>
            )}
            {step === 2 && (
              <motion.section key="pay" variants={variants} initial="enter" animate="center" exit="exit" className="space-y-7">
                ...
                <div className="space-y-2" role="radiogroup">
                  {([
                    ...
                    .map((o) => (
                    <label key={o.v} className={`flex min-h-[5.5rem] items-center gap-4 border px-5 py-4 transition-colors ${!o.ok ? "cursor-not-allowed opacity-50" : payment === o.v ? "cursor-pointer border-iodine bg-iodine-wash/60" : "cursor-pointer border-line/60 hover:border-line-strong"}`}>
                ...
                {user && maxPoints >= 100 && (
                  <div className="border border-line/60 bg-mist/60 px-5 py-4">
                    <label className="flex min-h-14 cursor-pointer items-center justify-between gap-3 text-[15.5px]">
                      <span className="flex items-center gap-2">
                        <CheckIcon size={16} className="text-ok" />
                        Utiliser mes {user.loyaltyPoints} points fidélité
                      </span>
                      <input type="checkbox" checked={usePoints} onChange={(e) => setUsePoints(e.target.checked)} className="h-5 w-5 accent-iodine" />
                    </label>
                    {usePoints && (
            ...
            {step === 3 && (
              <motion.section key="review" variants={variants} initial="enter" animate="center" exit="exit" className="space-y-6">
                ...
                <p className="text-[13px] text-muted">En confirmant, vous acceptez nos <Link href="/cgv" className="underline">conditions générales de vente</Link>.</p>
              </motion.section>
            )}
          </AnimatePresence>
        </div>
        ...
        <div className="mt-12 flex items-center justify-between gap-4">
          ...
          {step < 3 ? <button onClick={next} className="btn-solid min-h-14 px-8 text-[12.5px]">Continuer</button> : <button onClick={submit} disabled={pending} className="btn-solid min-h-14 min-w-64 text-[12.5px]"><LockIcon size={16} /> {pending ? "Traitement…" : `Confirmer · ${formatDT(total)}`}</button>}
        </div>
      </div>

      <aside className="lg:col-span-5"><div className="lg:sticky lg:top-28 border-t border-line/60 pt-7">
        <h3 className="mb-7 font-ant uppercase text-[26px] font-light text-carbon">Récapitulatif</h3>
        <ul className="max-h-72 space-y-3.5 overflow-y-auto pr-1">{cart.lines.map((l) => <li key={l.productId} className="flex items-center gap-3 text-[15px]"><div className="relative h-12 w-10 shrink-0 bg-line">{l.image && <Image src={l.image} alt="" fill sizes="40px" className="object-cover" />}<span className="absolute -right-1.5 -top-1.5 flex h-4 min-w-4 items-center justify-center bg-carbon px-1 text-[9px] text-canvas">{l.quantity}</span></div><span className="min-w-0 flex-1 truncate text-carbon">{l.name}</span><span className="tabular-nums text-carbon">{formatDT(l.priceMillimes * l.quantity)}</span></li>)}</ul>
